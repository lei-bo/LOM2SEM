...
class ArgParser:
    def __init__(self):
        ...
        parser.add_argument("--out", default='stdout.txt')
    ...
    def parse_args(self, verbose=False, use_random_seed=True):
        ...
        args.update(config)
        args = Namespace(**args)
        args.checkpoint_dir = f"./checkpoints/{splitext(os.path.basename(args.config))[0]}"
        os.makedirs(args.checkpoint_dir, exist_ok=True)
        args.train_record_path = f"{args.checkpoint_dir}/train_record.pkl"
        args.test_result_path = f"{args.checkpoint_dir}/test_result"
